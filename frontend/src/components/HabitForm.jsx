...
export default function HabitForm({method, habit}){
    ...
    const handleSubmit = async (e)=>{
        ...
        if (method === 'Create'){
            ...
            try{
                const res = await api.put(`/api/habits/${habit.id}/`, {title, color})
                navigate('/')
            } catch(error){
                alert(error)
            } finally{
    ...
    return (
        <div className='create-form-container'>
            <form className='habit-form' onSubmit={handleSubmit}>
                ...
                <button className="create-form-button" type="submit" disabled={loading}>{method.toUpperCase()}</button>
